...
--evaluator A|B|aggregate   Oracle 子角色；三 session 模式下分别跑
--oracle-serial             同一 session 内先后扮演 A/B 再仲裁（简化但 context 隔离较弱）
...
| 运行方式 | 同 Agent 继续跑 | **必须新开 context**（新 session / 新对话） |

## 执行步骤

### Step 1：强制 context 隔离

如果检测到当前 session 刚跑完 `/impl` 或 `/run-tasks`，**主动建议用户开新对话**：

```
⚠️  检测到当前 session 内刚执行过 /impl 或 /run-tasks。
对抗式评估要求 Evaluator 有独立 context，否则容易继承 Generator 的偏见。

建议：
1. 打开新的 Claude Code 对话 / 新标签页
2. 在新 session 里重新执行 /adversarial-review
3. 如果无法新开 session，请确认你理解偏见风险并输入 --force-same-context
```

用户若强行 `--force-same-context`，继续但**在最终报告顶部标注："本次评估 context 可能受污染，结果仅供参考"**。
...
**简化方式：`--oracle-serial`（同 session 内）**
