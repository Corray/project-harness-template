...
#### 7.3.5 异步轮询协议（wait: true 阶段共用）

**禁止同步 sleep + 轮询循环**——会让 Claude 整段被同步占住，浪费 context 和时间窗。

**正确做法**（Bash run_in_background + 完成通知）：

1. 调一次 `mcp__jenkins__get_build_status`，已终态 → 直接处理；否则进 2
2. 启动 Bash 后台命令：`sleep $poll_interval_seconds`，**`run_in_background=true`**
3. Claude 不阻塞——可以输出"⏳ {stage} 已触发 (#{build})，{N} 秒后轮询"让用户知道在等
4. 后台 sleep 完成 → Claude 收到自动完成通知 → 重新进入 1 步
5. 累计已等时间超过 `timeout_minutes` → 记 `status=TIMEOUT`，退出循环

**累计计时**：每次启动后台 sleep 前累加 `poll_interval_seconds` 到本阶段已等时长。不要在 bash 里跑长循环，**Claude 是循环主体，bash 只是"睡觉的工具"**。

**`/loop` 模式下的升级路径**：开发者如果在 `/loop` 模式下跑这条命令，可以用 `ScheduleWakeup(delaySeconds=poll_interval_seconds)` 替代 Bash sleep——能力等价但更省 token。普通 `/impl` 会话不能用 ScheduleWakeup（仅 /loop 动态模式可用）。

**重要的体感澄清（避免误以为"全异步"）**：

- 本协议的实际收益是 **节省 Claude context（不浪费 token）**，**不是缩短 /impl 总时长**
- Step 7 是 commit 之后的最后一步，wait: true 阶段在等的时候 Claude 没别的事能做——**用户体感仍是"卡住等"**，只是后台 sleep 期间 Claude 不消耗 token
- 真正想减少体感等待时间，**应该在 jenkins.yaml 把绝大多数 stage 配 wait: false**：
  - `package` 阶段：wait: true 合理（下游 `${stages.package.build_number}` 引用要求其完成）
  - `deploy` / `publish` / `release` 阶段：wait: false 合理（红线 26）—— 触发完拿 URL 即可，不阻塞 /impl
  - 链尾的 wait: true 阶段（无下游引用）：考虑改 wait: false，不必同 turn 内拿结果
- 如果当前 jenkins.yaml 把所有 stage 都配成 wait: true，请重新评估每条是否真的需要在 /impl 同 turn 内拿到终态
...
if wait（默认 true）:
    按 7.3.5 异步轮询协议轮询 mcp__jenkins__get_build_status，
    直到终态（SUCCESS / FAILURE / ABORTED / UNSTABLE）或累计超时（timeout_minutes 默认 30）
    超时记 status=TIMEOUT
```
...
for stage in stages:
    ...
    if stage.wait:
        # 按 7.3.5 异步轮询协议（不要同步 sleep 循环）
        status = poll_until_terminal_async(stage.job, build_number,
