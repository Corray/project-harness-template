...
**关于 `tasks.yaml` 完全缺失的边界情况**：
`/adversarial-review` 提供 **no-contract 降级模式**（D 维度权重 20→40 + 通用质量门 + 3 条 D 维度加强追问）。这是**降级**不是**补救**：上一条仍然成立——断言缺失就是抓不出对应的功能维度。降级模式存在的理由是"哪怕没合同，也比拒跑 / 假评强"，让历史 commit 和绕过 /impl 的手动改动有最低保险，同时把怀疑度往 D 维度倾斜（无合同时"走捷径"最容易滑过去）。**首选始终是 contract 模式**——/impl 小任务路径已经会自动写 ad-hoc tasks.yaml，让 no-contract 维持稀有。
